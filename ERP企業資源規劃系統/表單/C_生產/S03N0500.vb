...
#Region "@Form及功能操作@"
    Private Sub S03N0500_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        FillData("") '載入資料集
    End Sub
    Private Sub S03N0500_FormClosed(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        MySystemControl() '清除背景全域物件
    End Sub
    '尋找
    Public Overrides Sub FindWindows()
    End Sub
    Private Sub S03N0500_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        ControlFocus(TabPage2, e)
    End Sub
    Private Sub S03N0500_KeyUp(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyUp
...
#Region "DataGridView及ComboBox"
    ...
    Sub DataGridView_Basic()
        '欄位名稱
        Dim arrColName() As String = {"委外單號", "委外日期", "所屬工廠", "委外廠商"}
...
#Region "共用底層"
    ...
    Public Overrides Sub FillData(ByVal strSearch As String)
        ...
        '查詢語法
        strSSQL = "SELECT top " + PageCount.Text + " m.*  FROM 委外單主檔 AS m"
        strSSQL &= " WHERE 1=1 "
        strSSQL &= strSearch
        strSSQL &= " ORDER BY 委外單號 DESC"

        '顯示欄位
        arrRowName = {"委外單號", "委外日期", "所屬工廠", "委外廠商"}
    ...
    Public Overrides Sub SetControls(ByVal butStatus As Integer)
        ...
        Select Case butStatus
            ...
            Case 1 '新增模式
                ...
                strPage = FIRMCODE + Mid(CStr(委外日期.Value.Year - 1911), 2, 2) + 委外日期.Value.Month.ToString.PadLeft(2, "0")
                '委外單號.Text = ERP_AutoNo_委外單號(DNS, strPage, "委外單主檔", "委外單號")
    ...
    Public Overrides Function InsertData() As Boolean
        ...
        委外單號.Text = ERP_AutoNo_貨單序號(DNS, strPage, "委外單主檔", "委外單號")
        '代號是否有重複
        If dbDataCheck(DNS, "委外單主檔", "委外單號 = '" & 委外單號.Text & "'") = True Then MsgBox("系統訊息：【委外單號】已有相同之資料!!", MsgBoxStyle.Exclamation, "注意") : blnCheck = False
        ...
        '開啟儲存*****
        '過磅單主檔
        strIRow = "委外單號,委外日期,所屬工廠,委外廠商,"
        strIRow &= "建立人員,建立日期,修改人員,修改日期"

        strIValue = "'" & 委外單號.Text & "','" & strSystemToDate(委外日期.Value.ToShortDateString) & "','" & 所屬工廠.Text & "','" & 委外廠商.Text & "',"
        strIValue &= "'" & INI_Read("BASIC", "LOGIN", "UNAME") & "','" & INI_Read("BASIC", "LOGIN", "DATE") & "','" & INI_Read("BASIC", "LOGIN", "UNAME") & "','" & INI_Read("BASIC", "LOGIN", "DATE") & "'"

        blnCheck = dbInsert(DNS, "委外單主檔", strIRow, strIValue)
        ...
        For I As Integer = 0 To OutDataDGV.RowCount - 1 Step 1
            ...
            If strRowValue(0) <> "" Or strRowValue(1) <> "" Then
                strIRow = "委外單號,工令號碼,備註"
                strIValue = "'" & 委外單號.Text & "','" & strRowValue(0) & "','" & strRowValue(1) & "'"
                dbInsert(DNS, "委外單項目檔", strIRow, strIValue)
            End If
        Next
    ...
    Public Overrides Function UpdateData() As Boolean
        ...
        strWValue = "委外單號 = '" & 委外單號.Text & "'"

        blnCheck = dbEdit(DNS, "委外單主檔", strUValue, strWValue)

        FormKey = 委外單號.Text

        '先刪
        dbDelete(DNS, "委外單項目檔", "委外單號 = '" & 委外單號.Text & "'")
        '委外單項目檔
        For I As Integer = 0 To OutDataDGV.RowCount - 1 Step 1
            Dim strRowValue() As String = {OutDataDGV.Rows(I).Cells("工令號碼").Value,
                                           OutDataDGV.Rows(I).Cells("備註").Value
    ...
    Public Overrides Sub DeleteData()
        ...
        '--開啟異動--
        blnCheck = dbDelete(DNS, "委外單主檔", "委外單號 = '" & 委外單號.Text & "'")
        blnCheck = dbDelete(DNS, "委外單項目檔", "委外單號 = '" & 委外單號.Text & "'")
...
#Region "資料查詢"
    ...
    Sub Weigh(ByVal strKey1 As String)
        ...
        strSQL99 = "SELECT * FROM 委外單主檔"
        strSQL99 &= " WHERE 委外單號 = '" & strKey1 & "'"
    ...
    Sub WeighItem(ByVal strKey1 As String)
        ...
        strSSQL &= ",* FROM 委外單項目檔 AS d left JOIN 進貨單主檔  as m on m.工令號碼=d.工令號碼 "
...
#Region "物件異動選擇項"
    ...
    '依日期變換 委外單號
    Private Sub 委外日期_ValueChanged(sender As System.Object, e As System.EventArgs) Handles 委外日期.ValueChanged, 委外日期.ValueChanged
...
#Region "報表處理"
    ...
    Private Sub btnPreviewRep_Click(sender As System.Object, e As System.EventArgs) Handles btnPreviewRep.Click
        Try
            ...
            FRreport.Load(thisFolder + "委外單.frx")
            FRreport.SetParameterValue("ConnStr", INI_Read("CONFIG", "SET", "DNS"))
            FRreport.SetParameterValue("RepKey", 委外單號.Text)
            FRreport.Prepare()
