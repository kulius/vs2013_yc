...
Public Class S06R0300
#Region "單用變數"
    Dim FRreport As New Report
    Dim DNS As String = INI_Read("CONFIG", "SET", "DNS") 'DNS設定值
#End Region
    Private Sub S06R0300_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Me.Width = 900
        Me.Height = 600

        所屬年月起.Format = DateTimePickerFormat.Custom
        所屬年月起.CustomFormat = "yyyy 年 MM 月"

        所屬年月迄.Format = DateTimePickerFormat.Custom
        所屬年月迄.CustomFormat = "yyyy 年 MM 月"
    ...
    Private Sub btnPreviewRep_Click(sender As System.Object, e As System.EventArgs) Handles btnPreviewRep.Click
        ...
        FRreport.Load(ReportDir + "客戶應收帳款及產量明細.frx")
        FRreport.SetParameterValue("ConnStr", INI_Read("CONFIG", "SET", "DNS"))
        'FRreport.SetParameterValue("RepKey", 爐號.SelectedValue)
        FRreport.SetParameterValue("RepKey1", CStr(所屬年月起.Value.Year) + 所屬年月起.Value.Month.ToString.PadLeft(2, "0"))
        FRreport.SetParameterValue("RepKey2", CStr(所屬年月迄.Value.Year) + 所屬年月迄.Value.Month.ToString.PadLeft(2, "0"))
        FRreport.Prepare()
